...
#ifndef AvidaHardwareInstMemSpace_h
#define AvidaHardwareInstMemSpace_h
...
namespace Avida {
  namespace Hardware {
    ...
    class InstMemSpace : public Avida::InstructionSequence
    {
    private:
    ...
    public:
      InstMemSpace(const InstMemSpace& in_memory);
      InstMemSpace(const InstructionSequence& in_genome) : InstructionSequence(in_genome), m_flag_array(in_genome.GetSize()) { ; }
      explicit InstMemSpace(int size = 1)  : InstructionSequence(size), m_flag_array(size) { ClearFlags(); }
      InstMemSpace(const Apto::String& in_string) : InstructionSequence(in_string), m_flag_array(in_string.GetSize()) { ; }
      ~InstMemSpace() { ; }
      ...
      void operator=(const InstMemSpace& other_memory);
