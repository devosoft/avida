    ...
    void Process(cAvidaContext& ctx){
      ...
      for (int kk=0; kk < pop.GetLiveOrgList().GetSize(); kk++){
        ...
        if ( org->GetPhenotype().GetPrecalcIsViable() > 0) {
          viables++;
          double org_fitness = NAN;
          double org_metabolism = NAN;
          double org_gestation = NAN;
          
          // Only assign phenotype numbers if it has successfully created an offspring.
          if (0 < phen.GetNumDivides()) {
            org_fitness = phen.GetFitness();
            ...
            // cPhenotype has property num_divides and getter GetNumDivides.
            org_metabolism = phen.GetMerit().GetDouble();
            org_gestation = phen.GetGestationTime();
