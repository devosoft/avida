...
# Form implementation generated from reading ui file '/Users/kaben/Projects/Software/Avida/branch.kaben/avida--current--2.0--base-0/source/bindings/Boost.Python/AvidaGui2/pyOnePop_GraphView.ui'
#
...
class pyOnePop_GraphView(QWidget):
  def __init__(self,parent = None,name = None,fl = 0):
    ...
    if not name:
      self.setName("pyOnePop_GraphView")


    pyOnePop_GraphViewLayout = QVBoxLayout(self,0,6,"pyOnePop_GraphViewLayout")
    ...
    pyOnePop_GraphViewLayout.addLayout(layout210)

    self.m_graph_ctrl = pyGraphCtrl(self,"m_graph_ctrl")
    self.m_graph_ctrl.setSizePolicy(QSizePolicy(3,3,0,0,self.m_graph_ctrl.sizePolicy().hasHeightForWidth()))
    self.m_graph_ctrl.setMinimumSize(QSize(100,100))
    pyOnePop_GraphViewLayout.addWidget(self.m_graph_ctrl)
    ...
    self.m_combo_box = QComboBox(0,self,"m_combo_box")
    m_combo_box_font = QFont(self.m_combo_box.font())
    m_combo_box_font.setPointSize(11)
    self.m_combo_box.setFont(m_combo_box_font)
    layout262.addWidget(self.m_combo_box)
    spacer200 = QSpacerItem(10,10,QSizePolicy.Expanding,QSizePolicy.Minimum)
    layout262.addItem(spacer200)
    pyOnePop_GraphViewLayout.addLayout(layout262)

    self.languageChange()

    self.resize(QSize(153,202).expandedTo(self.minimumSizeHint()))
    self.clearWState(Qt.WState_Polished)


  def languageChange(self):
    self.setCaption(self.__tr("pyOnePop_GraphView"))
    self.textLabel9_2.setText(self.__tr("Graph"))
    self.toolButton31_2.setText(QString.null)
    self.m_combo_box.clear()
    self.m_combo_box.insertItem(self.__tr("Merit"))


  def __tr(self,s,c = None):
    return qApp.translate("pyOnePop_GraphView",s,c)
