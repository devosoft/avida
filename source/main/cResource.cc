...
const double cResource::UPDATE_STEP(1.0 / 10000.0);
const double cResource::EPSILON (1.0e-15);
const int cResource::PRECALC_DISTANCE(100);


void FlowMatter(cSpatialResElement &elem1, cSpatialResElement &elem2, 
                double inxdiffuse, 
...
cResource::cResource(int num_resources)
  : update_time(0.0)
...
cResource::cResource(const cResource &rc) {
  *this = rc;

  return;
}

const cResource &cResource::operator=(const cResource &rc) {
  SetSize(rc.GetSize());
...
void cResource::SetSize(int num_resources)
{
  ...
  for(int i = 0; i < num_resources; i++){
    spatial_resource_count[i] = new cSpatialRes(); 
  }
...
void cResource::SetCellResources(int cell_id, const Apto::Array<double> & res)
{
...
void cResource::Setup(cWorld* world, const int& res_index, const cString& name, const double& initial, const double& inflow, const double& decay,                  
        ...
        const double& in_init_plat, const double& in_threshold, const int& in_refuge, const bool& isgradient)
{
  ...
  if (in_geometry == nGeometry::GLOBAL) {
    ...
    cerr << "[cResource::Setup] Unknown resource geometry " << in_geometry << ".  Exiting.";
    exit(2);
  }
  ...
  if (in_geometry == nGeometry::GLOBAL) {
    ...
    if (isgradient) {
      delete dynamic_resources[res_index];
      dynamic_resources[res_index] = new cGradientRes(world, in_peakx, in_peaky, in_height, in_spread, in_plateau, in_decay,                                
...
void cResource::SetGradientRes(cAvidaContext& ctx, cWorld* world, const int& res_id, const int& peakx, const int& peaky,
                      ...
                      const double& plat_val, const double& threshold, const int& refuge) 
{
  ...
  dynamic_resources[res_id]->SetGradPeakX(peakx);
  dynamic_resources[res_id]->SetGradPeakY(peaky);
  dynamic_resources[res_id]->SetGradHeight(height);
  dynamic_resources[res_id]->SetGradSpread(spread);
  dynamic_resources[res_id]->SetGradPlateau(plateau);
  dynamic_resources[res_id]->SetGradInitialPlat(plat_val);
  dynamic_resources[res_id]->SetGradDecay(decay);
  dynamic_resources[res_id]->SetGradMaxX(max_x);
  dynamic_resources[res_id]->SetGradMaxY(max_y);
  dynamic_resources[res_id]->SetGradMinX(min_x);
  dynamic_resources[res_id]->SetGradMinY(min_y);
  dynamic_resources[res_id]->SetGradMoveScaler(move_a_scaler);
  dynamic_resources[res_id]->SetGradUpdateStep(updatestep);

  dynamic_resources[res_id]->SetGradIsHalo(halo);
  dynamic_resources[res_id]->SetGradHaloInnerRad(halo_inner_radius);
  dynamic_resources[res_id]->SetGradHaloWidth(halo_width);
  dynamic_resources[res_id]->SetGradHaloX(halo_anchor_x);
  dynamic_resources[res_id]->SetGradHaloY(halo_anchor_y);
  dynamic_resources[res_id]->SetGradMoveSpeed(move_speed);
  dynamic_resources[res_id]->SetGradPlatInflow(plateau_inflow);
  dynamic_resources[res_id]->SetGradPlatOutflow(plateau_outflow);
  dynamic_resources[res_id]->SetGradConeInflow(cone_inflow);
  dynamic_resources[res_id]->SetGradConeOutflow(cone_outflow);
  dynamic_resources[res_id]->SetGradientInflow(gradient_inflow);
  dynamic_resources[res_id]->SetGradPlatIsCommon(is_plateau_common);
  dynamic_resources[res_id]->SetGradFloor(floor);
  dynamic_resources[res_id]->SetGradHabitat(habitat);
  dynamic_resources[res_id]->SetGradMinSize(min_size);
  dynamic_resources[res_id]->SetGradMaxSize(max_size);
  dynamic_resources[res_id]->SetGradConfig(config);
  dynamic_resources[res_id]->SetGradCount(count);
  dynamic_resources[res_id]->SetGradResistance(resistance);
  dynamic_resources[res_id]->SetGradThreshold(threshold);
  dynamic_resources[res_id]->SetGradRefuge(refuge);
  
  dynamic_resources[res_id]->ResetGradRes(ctx, worldx, worldy);
}

void cResource::SetGradientPlatInflow(const int& res_id, const double& inflow) 
{
  assert(res_id >= 0 && res_id < resource_count.GetSize());
  assert(spatial_resource_count[res_id]->GetSize() > 0);
  dynamic_resources[res_id]->SetGradPlatInflow(inflow);
}

void cResource::SetGradientPlatOutflow(const int& res_id, const double& outflow) 
{
  assert(res_id >= 0 && res_id < resource_count.GetSize());
  assert(spatial_resource_count[res_id]->GetSize() > 0);
  dynamic_resources[res_id]->SetGradPlatOutflow(outflow);
}

void cResource::SetGradientConeInflow(const int& res_id, const double& inflow) 
{
  assert(res_id >= 0 && res_id < resource_count.GetSize());
  assert(spatial_resource_count[res_id]->GetSize() > 0);
  dynamic_resources[res_id]->SetGradConeInflow(inflow);
}

void cResource::SetGradientConeOutflow(const int& res_id, const double& outflow) 
{
  assert(res_id >= 0 && res_id < resource_count.GetSize());
  assert(spatial_resource_count[res_id]->GetSize() > 0);
  dynamic_resources[res_id]->SetGradConeOutflow(outflow);
}

void cResource::SetGradientInflow(const int& res_id, const double& inflow) 
{
  assert(res_id >= 0 && res_id < resource_count.GetSize());
  assert(spatial_resource_count[res_id]->GetSize() > 0);
  dynamic_resources[res_id]->SetGradientInflow(inflow);
}

void cResource::SetGradPlatVarInflow(const int& res_id, const double& mean, const double& variance, const int& type) 
{
  assert(res_id >= 0 && res_id < resource_count.GetSize());
  assert(spatial_resource_count[res_id]->GetSize() > 0);
  dynamic_resources[res_id]->SetGradPlatVarInflow(mean, variance, type);
}

void cResource::SetPredatoryResource(const int& res_id, const double& odds, const int& juvsper) 
{
  assert(res_id >= 0 && res_id < resource_count.GetSize());
  assert(spatial_resource_count[res_id]->GetSize() > 0);
  dynamic_resources[res_id]->SetPredatoryResource(odds, juvsper);
}

void cResource::SetProbabilisticResource(cAvidaContext& ctx, const int& res_id, const double& initial, const double& inflow, 
                                              const double& outflow, const double& lambda, const double& theta, const int& x, const int& y, const int& count) 
{
  assert(res_id >= 0 && res_id < resource_count.GetSize());
  assert(spatial_resource_count[res_id]->GetSize() > 0);
  dynamic_resources[res_id]->SetProbabilisticResource(ctx, initial, inflow, outflow, lambda, theta, x, y, count);
}
 ...
 */
int cResource::GetResourceCountID(const cString& res_name)
{
...
double cResource::GetInflow(const cString& name)
{
...
void cResource::SetInflow(const cString& name, const double _inflow)
{
  int id = GetResourceCountID(name);
  if (id == -1) return;
...
double cResource::GetDecay(const cString& name)
{
  int id = GetResourceCountID(name);
  if (id == -1) return -1;
...
void cResource::SetDecay(const cString& name, const double _decay)
{
  int id = GetResourceCountID(name);
  if (id == -1) return;
...
void cResource::Update(double in_time) 
{ 
  update_time += in_time;
...
const Apto::Array<double> & cResource::GetResources(cAvidaContext& ctx) const
{
  DoUpdates(ctx); 
  return resource_count;
}
 
const Apto::Array<double> & cResource::GetCellResources(int cell_id, cAvidaContext& ctx) const
...
const Apto::Array<double> & cResource::GetFrozenResources(cAvidaContext&, int cell_id) const
// Get amount of the resource for a given cell in the grid.  If it is a
// global resource pass out the entire content of that resource.
...
double cResource::GetFrozenCellResVal(cAvidaContext& ctx, int cell_id, int res_id) const
...
double cResource::GetCellResVal(cAvidaContext& ctx, int cell_id, int res_id) const
...
const Apto::Array<int> & cResource::GetResourcesGeometry() const
{
  return geometry;
}

const Apto::Array< Apto::Array<double> > &  cResource::GetSpatialRes(cAvidaContext& ctx)
{
...
void cResource::Modify(cAvidaContext& ctx, const Apto::Array<double> & res_change)
{
...
void cResource::Modify(cAvidaContext& ctx, int res_index, double change)
{
...
void cResource::ModifyCell(cAvidaContext& ctx, const Apto::Array<double> & res_change, int cell_id)
{
  assert(resource_count.GetSize() == res_change.GetSize());
...
double cResource::Get(cAvidaContext& ctx, int res_index) const
{
  assert(res_index < resource_count.GetSize());
  DoUpdates(ctx);
...
void cResource::Set(cAvidaContext& ctx, int res_index, double new_level)
{
  assert(res_index < resource_count.GetSize());
  DoUpdates(ctx);
  if (geometry[res_index] == nGeometry::GLOBAL || geometry[res_index]==nGeometry::PARTIAL) {
...
void cResource::ResizeSpatialGrids(int in_x, int in_y)
{
  for (int i = 0; i < resource_count.GetSize(); i++) {
...
int cResource::GetCurrPeakX(cAvidaContext& ctx, int res_id) const
{ 
  DoUpdates(ctx);
  return dynamic_resources[res_id]->GetCurrPeakX();
}

int cResource::GetCurrPeakY(cAvidaContext& ctx, int res_id) const
{ 
  DoUpdates(ctx);
  return dynamic_resources[res_id]->GetCurrPeakY();
}

int cResource::GetFrozenPeakX(cAvidaContext&, int res_id) const
{ 
  return dynamic_resources[res_id]->GetCurrPeakX();
}

int cResource::GetFrozenPeakY(cAvidaContext&, int res_id) const
{ 
  return dynamic_resources[res_id]->GetCurrPeakY();
}

Apto::Array<int>* cResource::GetWallCells(int res_id)
{
  return dynamic_resources[res_id]->GetWallCells();
}

int cResource::GetMinUsedX(int res_id)
{
  return dynamic_resources[res_id]->GetMinUsedX();
}

int cResource::GetMinUsedY(int res_id)
{
  return dynamic_resources[res_id]->GetMinUsedY();
}

int cResource::GetMaxUsedX(int res_id)
{
  return dynamic_resources[res_id]->GetMaxUsedX();
}

int cResource::GetMaxUsedY(int res_id)
{
  return dynamic_resources[res_id]->GetMaxUsedY();
}

///// Private Methods /////////
void cResource::DoUpdates(cAvidaContext& ctx, bool global_only) const
{ 
...
void cResource::ReinitializeResources(cAvidaContext& ctx, double additional_resource)
{
  for(int i = 0; i < resource_name.GetSize(); i++) {
 ...
 */
int cResource::GetResourceByName(cString name) const
{
  int result = -1;
