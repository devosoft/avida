...
class cActionSetGradPlatVarTailInflow : public cAction
{
private:
  cString m_res_name;
  double m_mean;
...
public:
  cActionSetGradPlatVarTailInflow(cWorld* world, const cString& args, Feedback&) : cAction(world, args), m_res_name(""), m_mean(0.0), m_variance(0.0)
  {
    cString largs(args);
    if (largs.GetSize()) m_res_name = largs.PopWord();
    if (largs.GetSize()) m_mean = largs.PopWord().AsDouble();
  ...
  void Process(cAvidaContext& ctx)
  {
    m_world->GetPopulation().SetGradPlatVarTailInflow(m_res_name, m_mean, m_variance);        
  } 
};
...
void RegisterEnvironmentActions(cActionLibrary* action_lib)
{
  ...
  action_lib->Register<cActionSetGradPlatVarTailInflow>("SetGradPlatVarTailInflow");
