...
public:
  ...
  const ResAmount& GetCellResources(int cell_id, cAvidaContext& ctx); 
  const ResAmount& GetFrozenResources(cAvidaContext& ctx, int cell_id);
  ...
  const ResAmount& GetDemeResources(int deme_id, cAvidaContext& ctx); 
...
public:
  ...
  const AResAmount& GetAVResources(cAvidaContext& ctx, int av_num = 0);
