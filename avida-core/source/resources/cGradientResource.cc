...
void cGradientResourceAcct::SetDeadlyRes(double odds)
{
  // we don't call this for walls and hills because they never move
  if (m_deadly) {
    for (int i = 0; i < m_plateau_cell_IDs.GetSize(); i ++) {
...
void cGradientResourceAcct::UpdateProbablisticRes()
{
  if (m_plateau_outflow > 0 || m_plateau_inflow > 0) {
...
void cGradientResource::ResetUsedBounds()
{
  m_min_usedx = -1;
  m_min_usedy = -1;
  m_max_usedx = -1;
...
void cGradientResourceAcct::ClearExistingProbRes()
{
