...
{
  const cResourceRegistry & res_reg = m_world->GetEnvironment().GetResourceRegistry();
  ...
  Print(res_reg.GetSize()+3, 2, "Reactions associated with Resource ");
  Print(res_reg.GetSize()+4, 2, "--Name--  --Num Rxns (last update)--  --");
  ...
  {
    Print(i+1, 1, res_reg.GetResource(i)->GetName());
    Print(i+1, 12, "%7.2f", res_reg.GetResource(i)->GetInflow());
    Print(i+1, 24, "%7.2f", res_reg.GetResource(i)->GetOutflow());
  }
  if (info.GetPauseLevel()) {
    Print(3, 58, "P");
    Print(4, 55, "N");
  ...
  Box(res_reg.GetSize()+2, 0, Height()-res_reg.GetSize()-2, Width(), true);
...
void cEnvironmentScreen::UpdateResource(cAvidaContext& ctx)
{
  ...
  Print(res_selection+1, 1, res_reg.GetResource(res_selection)->GetName());
  Print(res_selection+1, 12, "%7.2f", res_reg.GetResource(res_selection)->GetInflow());
  Print(res_selection+1, 24, "%7.2f", res_reg.GetResource(res_selection)->GetOutflow());
  ...
  for(int i=0; i<rxn_lib.GetSize(); i++) {
    for(int j=0; j<rxn_lib.GetReaction(i)->GetProcesses().GetSize(); j++) {
      if(rxn_lib.GetReaction(i)->GetProcesses().GetPos(j)->GetResource() != NULL)
      {
        ...
        cout << "BDB part 2 " << res_reg.GetResource(res_selection)->GetName() << endl;
        if(rxn_lib.GetReaction(i)->GetProcesses().GetPos(j)->GetResource()->GetName() ==
           res_reg.GetResource(res_selection)->GetName()) {
          int reactions = 0;
  ...
  SetBoldColor(COLOR_WHITE);
  Print(res_reg.GetSize()+3, 37, "%s", static_cast<const char*>(res_reg.GetResource(res_selection)->GetName()));
  Print(res_reg.GetSize()+3, res_reg.GetResource(res_selection)->GetName().GetSize()+37, ":");
  Print(res_reg.GetSize()+3, res_reg.GetResource(res_selection)->GetName().GetSize()+38, "        ");
...
void cEnvironmentScreen::UpdateReaction(cAvidaContext& ctx)
{
  ...
  for(int i = 0; i < num_processes; i++) {
    ...
    for(int j=0; j < res_reg.GetSize(); j++) {
      if (res_reg.GetResource(j)->GetName() == cur_resource->GetName()) {
    ...
    case KEY_DOWN:
      if(mode==ENVIRONMENT_MODE_RESOURCE ) {
        ...
        if (num_resources > 0) {
          ...
          Print(last_selection+1, 1, res_reg.GetResource(last_selection)->GetName());
          Print(last_selection+1, 12, "%7.2f", res_reg.GetResource(last_selection)->GetInflow());
          Print(last_selection+1, 24, "%7.2f", res_reg.GetResource(last_selection)->GetOutflow());
        }
      }
