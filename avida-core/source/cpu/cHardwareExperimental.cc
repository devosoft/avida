...
void cHardwareExperimental::PrintMiniTraceStatus(cAvidaContext& ctx, ostream& fp)
{
  ...
  for (int i = 0; i < resource_reg.GetSize(); i++) {
    if (resource_reg.GetResource(i)->GetHabitat() == 2) {
    ...
    else if (resource_reg.GetResource(i)->GetHabitat() == 1) {
...
bool cHardwareExperimental::Inst_IfNest(cAvidaContext& ctx) // Execute next if org on nest
{
  ...
  const cResourceRegistry& resource_reg = m_world->GetEnvironment().GetResourceRegistry();
  for (int i = 0; i < resource_reg.GetSize(); i++) {
    if (resource_reg.GetResource(i)->GetHabitat() == 3 || resource_reg.GetResource(i)->GetHabitat() == 4) {
      double cell_res = 0.0;
...
bool cHardwareExperimental::Inst_SenseResQuant(cAvidaContext& ctx)
{
  ...
  if (res_req < resource_reg.GetSize() && res_req > 0 && resource_reg.GetResource(res_req)->GetHabitat() != 3) {
    res_sought = res_req; 
  }
  ...
  // otherwise, we sum across all the food resources in the cell
    ...
    for (int i = 0; i < resource_reg.GetSize(); i++) {
      if (resource_reg.GetResource(i)->GetHabitat() == 0 || resource_reg.GetResource(i)->GetHabitat() > 5) {
...
bool cHardwareExperimental::Inst_SenseNest(cAvidaContext& ctx)
{
  ...
  if (nest_id < 0 || nest_id >= resource_reg.GetSize() || !resource_reg.GetResource(nest_id)->IsNest()) {
    for (int i = 0; i < resource_reg.GetSize(); i++) {
      if (resource_reg.GetResource(i)->IsNest()) {
        if (m_use_avatar) cell_res = m_organism->GetOrgInterface().GetAVResourceVal(ctx, i);
        else if (!m_use_avatar) cell_res = m_organism->GetOrgInterface().GetResourceVal(ctx, i);
        if (cell_res >= 1) {
          nest_id = i;
...
bool cHardwareExperimental::Inst_SenseResDiff(cAvidaContext& ctx) 
{
  ...
  const int res_req = m_threads[m_cur_thread].reg[FindModifiedRegister(rBX)].value;
  if (res_req < m_world->GetEnvironment().GetResourceRegistry().GetSize() && res_req > 0) {  
    res_sought = res_req; 
  }
...
bool cHardwareExperimental::Inst_CollectEdible(cAvidaContext& ctx)
{
  ...
  if (absorb_type == 1) {
    ...
    for (int i = resource_reg.GetSize(); i > 0 ; i--) {
      if (resource_reg.GetResource(i - 1)->GetHabitat() == 0 || resource_reg.GetResource(i - 1)->GetHabitat() > 5) {
        ...
        if (cell_res >= resource_reg.GetResource(i - 1)->GetThreshold()) {
          res_id = i - 1;
          break;
        }
      }
...
bool cHardwareExperimental::Inst_DepositResource(cAvidaContext& ctx)
{
  ...
  if (stored_res >= resource_amount && resource_amount > 0) {
    ...
    for (int i = 0; i < resource_reg.GetSize(); i++) {
      if (resource_reg.GetResource(i)->GetHabitat() == 4) {
        double cell_res = 0.0;
        if (m_use_avatar) cell_res = m_organism->GetOrgInterface().GetAVResourceVal(ctx, i);
        else if (!m_use_avatar) cell_res = m_organism->GetOrgInterface().GetResourceVal(ctx, i);
        if (cell_res > resource_reg.GetResource(i)->GetThreshold()) {
...
bool cHardwareExperimental::Inst_NopCollectEdible(cAvidaContext& ctx)
{
  ...
  if (res_id != -1) {
    ...
    double threshold = resource_reg.GetResource(res_id)->GetThreshold();
